...
def _create_pinocchio_model(articulation) -> PinocchioModel:
    xml = articulation._export_kinematics_chain_urdf_string()
    model = PinocchioModel(xml, articulation.scene.get_config().gravity)
    model.set_joint_order(
        [
            f"joint_{j.get_child_link().get_index()}"
            for j in articulation.get_active_joints()
        ]
    )
    model.set_link_order([f"link_{l.get_index()}" for l in articulation.get_links()])
    return model
